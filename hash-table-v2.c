...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
        ...
        int e = pthread_mutex_init(&entry->mutex1, NULL);
        if (e != 0) exit(e);
                             ...
                             uint32_t value)
{
    ...
    int e = pthread_mutex_lock(&hash_table_entry->mutex1);
    if (e != 0) exit(e);
	...
	if (list_entry != NULL) {
        ...
        e = pthread_mutex_unlock(&hash_table_entry->mutex1);
        if (e != 0) exit(e);
		return;
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
        ...
        int e = pthread_mutex_destroy(&entry->mutex1);
        if (e != 0) exit(e);
	}
	free(hash_table);
}
