...
struct hash_table_v1 *hash_table_v1_create()
{
    ...
    int e = pthread_mutex_init(&hash_table->mutex1, NULL);
    if(e!=0) exit(e);
	return hash_table;
}
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
    int e = pthread_mutex_lock(&hash_table->mutex1);
    if(e!=0) exit(e);
	...
	if (list_entry != NULL) {
        ...
        e = pthread_mutex_unlock(&hash_table->mutex1);
        if(e!=0) exit(e);
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
    ...
    int e = pthread_mutex_destroy(&hash_table->mutex1);
    if(e != 0) exit(e);
	free(hash_table);
}
